...
function ROICalculator() {
  ...
  // 3. Marges additionnelles sur OTC/para : +6% sur le CA OTC/para
  const margesGain = Math.round(inputs.caOtc * 0.06);
  // 4. Ruptures évitées : 90% des ruptures évitables × 80€ de CA moyen
  const rupturesGain = Math.round(inputs.nbRuptures * 80 * 0.90);
  // 5. Péremptions évitées : 0.5% du stock / an (entre 500€ et 2000€ selon taille)
  const peremptionsGain = Math.round(inputs.valeurStock * 0.005);
  // 6. CA protégé face à la concurrence online : 2.5% du CA OTC/para
  const prixGain = Math.round(inputs.caOtc * 0.025);
